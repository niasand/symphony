#!/usr/bin/env bash
# Symphony happyclaw Orchestrator daemon wrapper.
# Same as symphony-daemon.sh but uses WORKFLOW-happyclaw.md and port 3102.
set -euo pipefail
...
# ── Start orchestrator with happyclaw workflow ──
cd "$ELIXIR_DIR"
exec "$ELIXIR_DIR/bin/symphony" \
  --i-understand-that-this-will-be-running-without-the-usual-guardrails \
  --port 3102 \
  "$ELIXIR_DIR/WORKFLOW-happyclaw.md"
