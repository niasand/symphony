#!/usr/bin/env bash
# Symphony Ops Orchestrator daemon wrapper.
# Same as symphony-daemon.sh but uses WORKFLOW-ops.md and port 3101.
set -euo pipefail
...
# ── Start orchestrator with ops workflow ──
cd "$ELIXIR_DIR"
exec "$ELIXIR_DIR/bin/symphony" \
  --i-understand-that-this-will-be-running-without-the-usual-guardrails \
  --port 3101 \
  "$ELIXIR_DIR/WORKFLOW-ops.md"
