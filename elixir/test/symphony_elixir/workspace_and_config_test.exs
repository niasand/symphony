defmodule SymphonyElixir.WorkspaceAndConfigTest do
  ...
  test "config reads defaults for optional settings" do
    _previous_bitable_token = System.get_env("SYMPHONY_BITABLE_APP_TOKEN")
  ...
  test "config resolves $VAR references for env-backed secret and path values" do
    ...
    api_key_env_var = "SYMP_BITABLE_APP_TOKEN_#{System.unique_integer([:positive])}"
    ...
    write_workflow_file!(Workflow.workflow_file_path(),
      tracker_bitable_app_token: "$#{api_key_env_var}",
      workspace_root: "$#{workspace_env_var}",
      codex_command: "#{codex_bin} app-server"
    )

    config = Config.settings!()
    assert config.tracker.bitable_app_token == api_key
  ...
  test "config no longer resolves legacy env: references" do
    ...
    write_workflow_file!(Workflow.workflow_file_path(),
      tracker_bitable_app_token: "env:#{api_key_env_var}",
      workspace_root: "env:#{workspace_env_var}"
    )

    config = Config.settings!()
    assert config.tracker.bitable_app_token == "env:#{api_key_env_var}"
  ...
  test "schema parse normalizes policy keys and env-backed fallbacks" do
    ...
    previous_bitable_token = System.get_env("SYMPHONY_BITABLE_APP_TOKEN")

    System.delete_env(missing_workspace_env)
    System.put_env(empty_secret_env, "")
    System.delete_env(missing_secret_env)
    System.delete_env("SYMPHONY_BITABLE_APP_TOKEN")
    ...
    on_exit(fn ->
      ...
      restore_env("SYMPHONY_BITABLE_APP_TOKEN", previous_bitable_token)
    end)

    assert {:ok, settings} =
             Schema.parse(%{
               tracker: %{bitable_app_token: "$#{empty_secret_env}"},
               workspace: %{root: "$#{missing_workspace_env}"},
               codex: %{approval_policy: %{reject: %{sandbox_approval: true}}}
             })

    assert settings.tracker.bitable_app_token == nil
    ...
    assert {:ok, settings} =
             Schema.parse(%{
               tracker: %{bitable_app_token: "$#{missing_secret_env}"},
               workspace: %{root: ""}
             })
